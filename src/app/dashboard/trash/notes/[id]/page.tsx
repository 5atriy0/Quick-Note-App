...
export default function TrashNotePage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const { trashedNotes, batchRestore, batchPermanentDelete } = useNotes();
  ...
  return (
    <div className="flex h-full w-full">
      ...
      <div className="w-full md:flex-1 h-full overflow-hidden">
        <NoteEditor noteId={resolvedParams.id} returnTo="/dashboard/trash" />
      </div>
    </div>
  );
}
